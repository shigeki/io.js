...
our %config = (
  ...
  builddir => ".",
  cc => "cl",
  cflags => [ "-W3 -wd4090 -Gs0 -GF -Gy -nologo  /O2 -WX" ],
  conf_files => [ "Configurations/00-base-templates.conf", "Configurations/10-main.conf", "Configurations/shared-info.pl" ],
  cppflags => [  ],
  cross_compile_prefix => "",
  cxxflags => [  ],
  defines => [ "DSO_WIN32", "OPENSSL_SYS_WIN32", "WIN32_LEAN_AND_MEAN", "L_ENDIAN", "_CRT_SECURE_NO_DEPRECATE", "_WINSOCK_DEPRECATED_NO_WARNINGS", "UNICODE", "_UNICODE", "NDEBUG", "OPENSSL_NO_DYNAMIC_ENGINE", "OPENSSL_PIC" ],
  ...
  openssldir => "",
  options => " no-asan no-asm no-comp no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-shared no-ssl-trace no-ssl3 no-ssl3-method no-tls13downgrade no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl => "/usr/bin/perl",
  perlargv => [ "no-shared", "no-comp", "no-ssl3", "no-asm", "VC-WIN32" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
