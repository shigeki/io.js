...
TLSSocket.prototype.startTLS = function() {
  var args = getTLSSocketOptions(arguments);
  var options = args[0];
  var cb = args[1];

  assert(!this.isServer, 'Server TLSSocket cannot execute startTLS.');
  assert(this._requestCert, 'startTLS needs requestCert to be true.');
  ...
  // TODO(shigeki) other options need to be overwritten?
  if (options.session)
    this.setSession(options.session);

  if (options.servername)
    this.setServername(options.servername);

  this._start();

  this.on('secure', secureListner);

  this.once('end', onHangUp);
};
...
function getTLSSocketOptions(listArgs) {
  const argsLen = listArgs.length;
  ...
  options.NPNProtocols = NPN.NPNProtocols;
  tls.convertALPNProtocols(options.ALPNProtocols, ALPN);
  options.ALPNProtocols = ALPN.ALPNProtocols;
...
function secureListner(socket) {
  // Check the size of DHE parameter above minimum requirement
  // specified in options.
  var options = socket._tlsOptions;
  ...
  if (!verifyError && !socket.isSessionReused()) {
    ...
    verifyError = options.checkServerIdentity(options.hostname, cert);
  }
